    ...
    this.$nextTick(()=>{
      ...
      const generateData = () => {
        ...
        while (date <= new Date('2023-03-01')) {
          ...
          p_activity.push([date.getTime(), value1]);
          n_participant.push([date.getTime(), value2]);
          n_forks.push([date.getTime(), value3]);
          n_follows.push([date.getTime(), value4]);
          n_stars.push([date.getTime(), value5]);
          scores.push([date.getTime(), value6]);
          date.setMonth(date.getMonth() + 1); // 日期加一个月
        }
        return [p_activity, n_participant, n_forks,n_follows,n_stars,scores];
      };
      console.log(generateData())
      // 随机数据
      const [p_activity, n_participant, n_forks,n_follows,n_stars,scores] = generateData();
      // 配置项
      const option = {
        tooltip: {
          trigger: 'axis',
